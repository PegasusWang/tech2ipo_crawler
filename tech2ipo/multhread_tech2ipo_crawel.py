...
def get_articleurl_from_allpage(all_url):
    ...
    html_origin = requests.get(all_url).text
...
def parse_page(page_url):
    ...
    try:
        html_origin = requests.get(page_url, timeout=3).text
    except:
        return
...
def main():
    ...
    for i in range(thread_num):
        ...
        t.setDaemon(True)
        t.start()
    fresh_article_url_set = get_fresh_article_url_set()
    ...
    write_article_brief(fresh_article_url_set)
